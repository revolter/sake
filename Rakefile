...
def generate_docs
  ...
  sh "jazzy --clean --sdk macosx --xcodebuild-arguments -scheme,sake --skip-undocumented --no-download-badge"
end
...
def get_archive_url(version)
  'https://github.com/xcodeswift/sake/archive/' + "#{version}" + '.tar.gz'
end

def download_archive(url)
  # follow link, output errors, no progress meter
  `curl -LSs #{url} -o sake.tar.gz`
end

def get_checksum
  `shasum -a 256 sake.tar.gz | awk '{printf $1}'`
end

def update_formula(version)
  path = 'Formula/sake.rb'
...
desc "Bumps the version of sake. It creates a new tagged commit and archives the binary to be published with the release"
